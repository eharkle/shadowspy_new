...
def get_vertex_normals(V, F):
    ...
    vertex_normals = np.zeros(V.shape)

    # Calculate the count of triangles sharing each vertex
    unique, counts = np.unique(F, return_counts=True)
    vertex_triangle_counts = np.zeros(len(V), dtype=int)
    vertex_triangle_counts[unique] = counts

    # Repeat triangle normals for each vertex (each triangle has 3 vertices)
    vertex_triangle_normals = np.repeat(triangle_normals, 3, axis=0)

    # Accumulate triangle normals to vertices
    # unravelling F gives list of vertices (of len ntriangles * 3)
    # vertex_triangle_normals are repeated 3x since same for the same face
    # assign to each vertex in this list the normals related to the triangles where it appears
    np.add.at(vertex_normals, F.ravel(), vertex_triangle_normals)

    # Normalize vertex normals
    vertex_normals /= vertex_triangle_counts[:, np.newaxis]
    # assert np.max(np.linalg.norm(vertex_normals, axis=1)) == 1.
    vertex_normals = vertex_normals / np.linalg.norm(vertex_normals, axis=1)[:, np.newaxis]

    # set to outside pointing normals?
    vertex_normals[np.einsum('ik,ik->i', vertex_normals, V/np.linalg.norm(V, axis=1)[:, np.newaxis]) < 0] *= -1.
...
class TrimeshShapeModel(ShapeModel):
    ...
    def __init__(self, V, F, N=None, P=None, A=None, VN=None):
...
class CgalTrimeshShapeModel(TrimeshShapeModel):
    def _make_scene(self):
        if 'aabb' not in sys.modules:
            raise ImportError('failed to import AABB from aabb')
...
class EmbreeTrimeshShapeModel(TrimeshShapeModel):
    ...
    def _intersect1_2d(self, X, D):
        raise RuntimeError('intersect1_2d no implemented for EmbreeTrimeshShapeModel')
